...
public class VendaClienteRepository : IVendaClienteRepository
{
    ...
    public IReadOnlyCollection<Cliente> GetFor(Venda venda)
    {
    ...
    {
        using var command = commandFactory.Create("DELETE FROM Vendas_Clientes WHERE Id_Venda = @Id_Venda");
        command.AddParameter("@Id_Venda", venda.Id);
    ...
    {
        using var command = commandFactory.Create("INSERT INTO Vendas_Clientes (Id_Venda, Id_Cliente) VALUES (@Id_Venda, @Id_Cliente)");
        command.AddParameter("@Id_Venda", venda.Id);
